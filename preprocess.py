...
def generate_combined_dataset():
    ...
    result_df.to_excel('assets/combined_dataset.xlsx', index=False)


def process_all_songs():
    df = pd.read_excel('/home/marcotuiio/TaylorSwift_Guesser/assets/combined_dataset.xlsx')
    ...
    # Apply aggregation function to the 'tokens' column
    df['song_vector'] = df['tokens'].apply(lambda x: aggregate_vectors(x, model))
    df.to_excel('combined_dataset_preprocessed.xlsx', index=False)
